...
export function codexRoutes(ctx) {
  const { openai, providers, catalog, stats, provenance } = ctx;
  ...
  /** POST /responses: route by the routing-hint header; pass-through bodies are never decompressed. */
  async function responses(req, res) {
    ...
    const rewritten = rewriteResponsesRequest({ ...body, model: stripSuffix(model) }, profileFor(provider));
    const url = new URL(provider.responsesPath, provider.baseUrl);
    const payload = Buffer.from(JSON.stringify(rewritten));
    const entry = { client: 'codex', route: 'responses', model: rewritten.model, role, upstream: provider.name };
    let up;
